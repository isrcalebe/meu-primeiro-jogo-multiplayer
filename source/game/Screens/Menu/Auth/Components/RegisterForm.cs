...
public partial class RegisterForm : CompositeDrawable
{
    private readonly TextBox usernameTextBox;
    private readonly TextBox passwordTextBox;
    private readonly TextBox passwordConfirmTextBox;
    ...
    private readonly Bindable<string> passwordConfirmState;

    public RegisterForm()
    {
        usernameState = new Bindable<string>(string.Empty);
        passwordState = new Bindable<string>(string.Empty);
        passwordConfirmState = new Bindable<string>(string.Empty);
        ...
        [
            ...
            new FillFlowContainer
            {
                ...
                [
                    ...
                    passwordConfirmTextBox = new BasicTextBox
                    {
        ...
        passwordConfirmState.BindTo(passwordConfirmTextBox.Current);
    }

    private void onSubmit()
    {
        if (!passwordState.Value.Equals(passwordConfirmState.Value))
        {
            userStatusText.Text = "Passwords do not match!";
            userStatusText.Colour = Colour4.Red;
        }

        Logger.Log($"Data: {{ Username: {usernameState.Value}, Password: {passwordState.Value}, ConfirmPassword: {passwordConfirmState.Value} }}");
    }
}
